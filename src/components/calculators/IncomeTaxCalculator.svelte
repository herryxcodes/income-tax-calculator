...
<div class="calculator-wrapper">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    <div class="space-y-5">
      ...
      <!-- Employment Income -->
      <div>
        <label for="employment" class="block text-sm font-medium text-gray-700 mb-1">
          Employment Income
        </label>
        <div class="relative">
      ...
      <div>
        ...
        <p class="text-xs text-gray-400 mt-1">2025 max: $32,490 or 18% of income</p>
      </div>

      <!-- FHSA Contribution -->
      <div>
        <label for="fhsa" class="block text-sm font-medium text-gray-700 mb-1">
          FHSA Contribution
          <span class="text-gray-400 font-normal">(optional)</span>
        </label>
        <div class="relative flex gap-2">
          ...
          <button
            ...
            class="px-4 py-2.5 rounded-lg text-sm font-semibold bg-blue-600 text-white hover:bg-blue-700 cursor-pointer transition"
          >
            Max
          </button>
        </div>
        <p class="text-xs text-gray-400 mt-1">First Home Savings Account — $8,000/year max</p>
      </div>
      ...
      {#if showAdvanced}
        <div class="space-y-4 pl-3 border-l-2 border-blue-100">
          <div>
            <label for="capitalGains" class="block text-sm font-medium text-gray-700 mb-1">Capital Gains</label>
            <div class="relative">
              <span class="absolute left-3 top-2.5 text-gray-400">$</span>
              <input
                id="capitalGains"
            ...
            <p class="text-xs text-gray-400 mt-1">50% inclusion rate applies</p>
          </div>

          <div>
            <label for="eligibleDiv" class="block text-sm font-medium text-gray-700 mb-1">Eligible Dividends</label>
            <div class="relative">
              <span class="absolute left-3 top-2.5 text-gray-400">$</span>
              <input
                id="eligibleDiv"
          ...
          <div>
            <label for="ineligibleDiv" class="block text-sm font-medium text-gray-700 mb-1">Ineligible Dividends</label>
            <div class="relative">
              <span class="absolute left-3 top-2.5 text-gray-400">$</span>
              <input
                id="ineligibleDiv"
          ...
          <div>
            ...
            <p class="text-xs text-gray-400 mt-1">Rental, interest, pension, EI benefits, etc.</p>
          </div>
        </div>
      {/if}
    </div>
    ...
    <div>
      {#if result && result.totalIncome > 0}
        <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 space-y-5">
          <h2 class="text-lg font-semibold text-gray-900">
            Your 2025 Tax Estimate
            <span class="text-sm font-normal text-gray-500">({result.province.name})</span>
          </h2>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="bg-white rounded-xl p-4 shadow-sm">
              <p class="text-sm text-gray-500">After-Tax Income</p>
              <p class="text-2xl font-bold text-green-600">{formatCurrency(result.afterTaxIncome)}</p>
            </div>
            <div class="bg-white rounded-xl p-4 shadow-sm">
              <p class="text-sm text-gray-500">Average Tax Rate</p>
              <p class="text-xl font-bold text-gray-900">{formatPercent(result.averageRate)}</p>
            </div>
            <div class="bg-white rounded-xl p-4 shadow-sm">
              <p class="text-sm text-gray-500">Marginal Tax Rate</p>
              <p class="text-xl font-bold text-gray-900">{formatPercent(result.marginalRate)}</p>
            </div>
          </div>

          <!-- Tax Breakdown -->
          <div class="bg-white rounded-xl p-4 shadow-sm">
            <h3 class="text-sm font-semibold text-gray-700 mb-3">Tax Breakdown</h3>
            <div class="space-y-2 text-sm">
              ...
              <div class="flex justify-between">
                <span class="text-gray-600">{result.province.name} Tax</span>
                <span class="font-medium">{formatCurrency(result.provincialTax)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">{province?.slug === "quebec" ? "QPP" : "CPP"} Contributions</span>
              ...
              {#if result.cpp2Contributions > 0}
                <div class="flex justify-between pl-4">
                  <span class="text-gray-400 text-xs">Includes enhanced CPP on income above $71,300</span>
                </div>
              {/if}
              <div class="flex justify-between">
                <span class="text-gray-600">EI Premiums</span>
                <span class="font-medium">{formatCurrency(result.eiPremiums)}</span>
              </div>
              {#if result.qpipPremiums > 0}
                <div class="flex justify-between">
                  <span class="text-gray-600">QPIP Premiums</span>
          ...
          <div class="bg-white rounded-xl p-4 shadow-sm">
            <h3 class="text-sm font-semibold text-gray-700 mb-3">Federal Tax Brackets</h3>
                ...
                {#each result.federalBrackets as b}
                  {#if b.taxableInBracket > 0}
                    <tr class="border-b border-gray-50">
                      ...
                      <td class="py-1.5 text-right font-mono text-gray-700">{(b.rate * 100).toFixed(1)}%</td>
                      <td class="py-1.5 text-right font-mono text-gray-700">{formatCurrency(b.taxableInBracket)}</td>
                      <td class="py-1.5 text-right font-mono text-gray-700">{formatCurrency(b.taxInBracket)}</td>
                    </tr>
                  {/if}
                {/each}
              </tbody>
            ...
            <h3 class="text-sm font-semibold text-gray-700 mt-4 mb-3">{result.province.name} Tax Brackets</h3>
            <table class="w-full text-xs">
              <thead>
                <tr class="border-b border-gray-200 text-gray-500">
                  <th class="text-left py-1.5 font-medium">Bracket</th>
                ...
                {#each result.provincialBrackets as b}
                  {#if b.taxableInBracket > 0}
                    <tr class="border-b border-gray-50">
                      ...
                      <td class="py-1.5 text-right font-mono text-gray-700">{(b.rate * 100).toFixed(2)}%</td>
                      <td class="py-1.5 text-right font-mono text-gray-700">{formatCurrency(b.taxableInBracket)}</td>
                      <td class="py-1.5 text-right font-mono text-gray-700">{formatCurrency(b.taxInBracket)}</td>
                    </tr>
                  {/if}
          ...
          <div class="bg-white rounded-xl p-4 shadow-sm">
            <h3 class="text-sm font-semibold text-gray-700 mb-2">Income Split</h3>
          ...
          <div class="flex gap-3">
            <button
              ...
              class="flex-1 bg-white border border-gray-300 text-gray-700 rounded-lg px-4 py-2 text-sm font-medium hover:bg-gray-50 transition"
            >
              {shareButtonText}
            </button>
            <button
              ...
              class="flex-1 bg-blue-600 text-white rounded-lg px-4 py-2 text-sm font-medium hover:bg-blue-700 transition"
            >
          ...
          >
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-semibold text-gray-900 group-hover:text-green-700 transition">Ready to file your return?</p>
      ...
      {:else}
        <div class="bg-gray-50 rounded-2xl p-8 text-center">
          <p class="text-gray-400 text-lg">Enter your income to see your tax estimate</p>
          <p class="text-gray-300 text-sm mt-2">Your data never leaves your browser</p>
        </div>
      {/if}
    </div>
  </div>
  ...
  {#if showComparison && sortedResults.length > 0}
    <div class="mt-8">
      <h2 class="text-lg font-semibold text-gray-900 mb-4">All Provinces Compared</h2>
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b-2 border-gray-200">
              <th class="text-left py-3 px-3 font-semibold text-gray-700">Province</th>
              <th class="text-right py-3 px-3 font-semibold text-gray-700">Total Tax</th>
              <th class="text-right py-3 px-3 font-semibold text-gray-700">After-Tax</th>
              <th class="text-right py-3 px-3 font-semibold text-gray-700">Avg Rate</th>
              <th class="text-right py-3 px-3 font-semibold text-gray-700">Marginal</th>
            </tr>
          </thead>
          <tbody>
            {#each sortedResults as r}
              <tr
                class="border-b border-gray-100 {r.province.slug === selectedProvince
              ...
              >
                <td class="py-2.5 px-3">
                  ...
                  {#if r.province.slug === selectedProvince}
                    <span class="text-blue-600 text-xs ml-1">(selected)</span>
                  {/if}
                </td>
  ...
  <p class="mt-6 text-xs text-gray-400 text-center">
    ...
    Calculations based on 2025 CRA-published tax rates.
